...
class SubcategoryPreview extends Component {
    constructor(props){
        ...
        this.state = {
            ...
            completedSubcategory: false}
        ;
    }

    toggleCompleted(){
        this.setState(previousState => {
            return{ completedSubcategory: !previous.expandedSubcategory};
        });
    }

    toggleExpand(){
        this.setState(previousState=>{
            return {expandedSubcategory: !previousState.expandedSubcategory};
        });
    }

    renderButton(){
        if (this.props.mode === 'start'){
            if (this.state.completedSubcategory)
            {
                return(
                    <Button 
                        fontColor='white'
    ...
    renderModal(){
        const { title, description } = this.props.subcategory;
        const { avatarStyle, titleStyle, descriptionStyle, modalStyle } = styles;

        return (
            <Modal
                animationType="slide"
                visible={this.state.expandedSubcategory}
                transparent
